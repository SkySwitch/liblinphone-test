...
void Account::update () {
	if (mNeedToRegister){
		updatePushNotificationParameters();
		if (canRegister()){
			registerAccount();
...
void Account::onPushNotificationAllowedChanged (bool callDone) {
	bool pushReady = mParams->isPushNotificationAvailable();
	...
	if (pushReady) {
		...
		if (!computedPushParams.empty() && mCore->push_notification_enabled && (mParams->mPushNotificationAllowed || mParams->mRemotePushNotificationAllowed)) {
			if (contactUriParams.empty() || contactUriParams != computedPushParams) {
				...
				ms_message("Push notification information [%s] added to account [%p]", computedPushParams.c_str(), this->toC());
			}
		} else {
